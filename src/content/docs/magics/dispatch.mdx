...
- [ ] <span class="sr-only">Слушатель с модификатором once</span>
  ```html wrap
  <div x-data>
  ...
  ```
  > `.once` лишь ограничивает обработчик одним срабатыванием, а до соседнего элемента событие всё равно не дойдёт.
- [ ] <span class="sr-only">Слушатель без модификаторов</span>
  ```html wrap
  <div x-data>
    <span @notify="..."></span>
    <button @click="$dispatch('notify')">Уведомить</button>
  ...
  ```
  > Событие всплывает от кнопки к общему предку `div` и на соседний `span` не попадает.
- [x] <span class="sr-only">Слушатель с модификатором window</span>
  ```html wrap
  <div x-data>
    <span @notify.window="..."></span>
    <button @click="$dispatch('notify')">Уведомить</button>
  ...
  ```
  > Пользовательское событие в итоге всплывает до `window`, где его и перехватывает модификатор `.window`.
