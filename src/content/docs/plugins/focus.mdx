...
<Quiz id="focus-q1" title="Вопрос 1">

Как отключить автоматическую фокусировку элемента при захвате фокуса?

- [x] Добавить модификатор `.noautofocus`
  > `x-trap.noautofocus` захватывает фокус внутри элемента, но ничего не фокусирует сразу.
- [ ] Добавить модификатор `.disablefocus`
  > Такого модификатора у плагина Focus нет.
- [ ] Добавить модификатор `.nofocus`
  > Имя придумано — нужный модификатор называется `.noautofocus`.

</Quiz>

<Quiz id="focus-q2" title="Вопрос 2">

Как передать фокус следующему фокусируемому элементу?

- [x] `$focus.next()`
  > Магия `$focus` передаёт фокус следующему элементу методом `next()`.
- [ ] `x-focus.next`
  > Директивы `x-focus` не существует — плагин даёт директиву `x-trap` и магию `$focus`.
- [ ] `$focus.getNext()`
  > Такого метода у `$focus` нет.
