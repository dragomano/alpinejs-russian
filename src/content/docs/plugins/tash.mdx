---
title: Tash
description: Используем интерполяцию React, Vue или Angular с помощью Alpine JS
origin: https://github.com/markmead/alpinejs-tash
banner:
  content: |
    Этот плагин не является официальным и добавлен в эту документацию переводчиком.
---

import { Tabs, TabItem } from '@astrojs/starlight/components';
import { PackageManagers } from 'starlight-package-managers';

Если вы привыкли к способам записи переменных в React, Vue или Angular, то этот плагин поможет использовать более привычный синтаксис при отображении `{переменных}` Alpine JS 🚀.

## Установка

Вы можете использовать этот плагин, включив его из тега `<script>` или установив с помощью менеджера пакетов.

### Через CDN

Вы можете подключить CDN-сборку этого плагина с помощью тега `<script>`, только подключать нужно ДО основного JS-файла Alpine.

<Tabs>
  <TabItem label="jsDelivr CDN">
    ```html
    <!-- Tash Plugin -->
    <script defer src="https://cdn.jsdelivr.net/npm/alpinejs-tash@latest/dist/tash.min.js"></script>

    <!-- Alpine Core -->
    <script defer src="https://cdn.jsdelivr.net/npm/alpinejs@3/dist/cdn.min.js"></script>
    ```
  </TabItem>
  <TabItem label="unpkg CDN">
    ```html
    <!-- Tash Plugin -->
    <script defer src="https://unpkg.com/alpinejs-tash@latest/dist/tash.min.js"></script>

    <!-- Alpine Core -->
    <script defer src="https://unpkg.com/alpinejs@3/dist/cdn.min.js"></script>
    ```
  </TabItem>
</Tabs>

### Через менеджер пакетов

Вы можете установить Tash для использования внутри вашей сборки следующим образом:

<PackageManagers pkg="alpinejs-tash" icons={false} />

Затем инициализируйте его в своей сборке:

```js {2, 4}
import Alpine from 'alpinejs';
import tash from 'alpinejs-tash';

Alpine.plugin(tash);
Alpine.start();
```

## Использование

### Стиль React

```html live "x-tash"
<div x-data="{ name: 'Вася', age: 50, company: 'GitHub' }">
  <p x-tash="name, age, company">
    Привет, я {name}! Мне {age} лет и я работаю в {company}!
  </p>
</div>
```

### Стиль Vue

```html live "x-tash"
<div x-data="{ name: 'Вася', age: 50, company: 'GitHub' }">
  <p x-tash.vue="name, age, company">
    Привет, я {{ name }}! Мне {{ age }} лет и я работаю в {{ company }}!
  </p>
</div>
```

### Стиль Angular

```html live "x-tash"
<div x-data="{ name: 'Вася', age: 50, company: 'GitHub' }">
  <p x-tash.angular="name, age, company">
    Привет, я {{name}}! Мне {{age}} лет и я работаю в {{company}}!
  </p>
</div>
```
