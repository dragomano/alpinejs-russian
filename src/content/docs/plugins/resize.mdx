...
Визуальная область просмотра — это часть страницы, которая в данный момент видна на экране. В мобильных браузерах она может изменяться независимо от документа, когда появляются элементы управления браузера или экранная клавиатура. Используйте модификатор `.viewport`, чтобы реагировать на эти изменения с помощью `window.visualViewport`:

```html wrap "x-resize.viewport"
<div x-resize.viewport="width = $width; height = $height">
    ...
</div>
```

`$width` и `$height` содержат размеры визуальной области просмотра. В браузерах, где `window.visualViewport` недоступен, Alpine вместо этого отслеживает изменения документа.
