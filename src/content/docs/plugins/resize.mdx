...
Основным API для использования этого плагина является `x-resize`. Вы можете добавить `x-resize` к любому элементу в компоненте Alpine, и когда размер этого элемента будет изменён по какой-либо причине, будет выполнено выражение с двумя волшебными свойствами: `$width` и `$height`.
