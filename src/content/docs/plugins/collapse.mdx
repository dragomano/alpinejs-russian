...
<Quiz id="collapse-q1" title="Вопрос 1">

С помощью какой директивы задействуется этот плагин?

- [ ] `x-colapse`
  > Здесь опечатка: в названии директивы две буквы `l` — `x-collapse`.
- [ ] `x-spoiler`
  > Такой директивы в Alpine нет.
- [x] Правильных ответов нет
  > Плагин работает через `x-collapse`, а такого варианта среди ответов нет.

</Quiz>

<Quiz id="collapse-q2" title="Вопрос 2">

Как указать продолжительность перехода свёртывания/развёртывания в 10 секунд?

- [x] Добавить модификатор `.duration.10s`
  > Значение с суффиксом `s` задаёт длительность в секундах.
- [ ] Добавить модификатор `.duration.10ms`
  > Такая запись даст 10 миллисекунд — в тысячу раз меньше нужного.
- [ ] Добавить модификатор `.duration.10sec`
  > Единицы `sec` не существует — допустимы только `s` и `ms`.
