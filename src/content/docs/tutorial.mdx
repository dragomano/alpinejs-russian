...
1. ### Пример: Появление элемента
    ...
    ```
    Элемент появляется с анимацией прозрачности.
...
1. ### Пример: Загрузка постов
    ...
    ```html live
    <div x-data="{ posts: [] }" x-init="fetch('https://jsonplaceholder.typicode.com/posts')
        .then(response => response.json())
        .then(data => posts = data.slice(0, 10))">
        <ul>
    ...
    ```
    Данные из API отображаются в списке.
