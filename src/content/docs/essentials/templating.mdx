...
<Quiz id="templating-q1" title="Вопрос 1">
...
- [ ] 1 + 2
  > Значение `x-text` — это выражение JavaScript, а не литеральный текст.
- [ ] 12
  > Слагаемые — числа, поэтому они складываются, а не склеиваются как строки.
- [x] 3
  > Alpine вычисляет выражение `1 + 2` и подставляет результат в `<span>`.

</Quiz>

<Quiz id="templating-q2" title="Вопрос 2">

Какой из этих хелперов переходов не существует?

- [ ] `x-transition.opacity`
  > Модификатор `.opacity` существует и оставляет в переходе только изменение прозрачности.
- [x] `x-transition.break`
  > Такого модификатора у `x-transition` нет.
- [ ] `x-transition.scale`
  > Модификатор `.scale` существует и оставляет в переходе только масштабирование.
