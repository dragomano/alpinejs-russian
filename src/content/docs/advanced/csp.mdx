...
```html
<!-- ✅ Работают -->
<div x-data="{ items: ['a', 'b'], getMessage: () => 'Привет' }">
...
<!-- ✅ Работают -->
<div x-data="{ count: 42 }">
    <button x-on:click="console.log('Число:', count)">Показать число</button>
...
Некоторые расширенные возможности JavaScript не поддерживаются:
