...
- [ ] Только идентификатор элемента
  > Идентификатор — лишь один из возможных вариантов селектора.
- [ ] Якорь ссылки
  > Якоря к `x-teleport` отношения не имеют: значение уходит в `document.querySelector`.
- [x] Любой CSS-селектор
  > Значение передаётся в `document.querySelector`, поэтому подойдёт любой валидный CSS-селектор.
