...
Если вы хотите применить плавные переходы к поведению `x-show`, вы можете использовать её вместе с [`x-transition`](/directives/transition). Вот краткий пример того же компонента, что и выше, только с применёнными переходами:
