...
Если вы хотите использовать `$persist` с `Alpine.data`, то вам необходимо использовать стандартную функцию вместо функции-стрелки, чтобы Alpine мог связать пользовательский контекст `this` при первоначальной оценке области видимости компонента.
